...
import { Loader2, PawPrint, Send, Upload, Wand2 } from 'lucide-react';
...
export default function ChatPage() {
  ...
  useEffect(() => {
    const storedVoiceId = localStorage.getItem('clonedVoiceId');
    if (storedVoiceId) {
  ...
  useEffect(() => {
    if (scrollAreaRef.current) {
      ...
      requestAnimationFrame(() => {
        const lastMessage = scrollAreaRef.current?.querySelector('.message-item:last-child');
  ...
  const handleCloneVoice = async (audioDataUri: string) => {
    ...
    try {
      ...
      toast({
        title: 'Voice Cloned!',
        description: `Your teddy bear will now use a new voice!`,
      });
    } catch (error) {
    ...
    <div className="flex flex-col h-screen bg-muted/20">
      ...
      <main className="flex-1 p-4 sm:p-6 md:p-8 overflow-hidden">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl mx-auto h-full">
          ...
          <div className="lg:col-span-2 flex flex-col h-[calc(100vh-150px)]">
            <Card className="flex-1 flex flex-col overflow-hidden">
                ...
                <form onSubmit={handleSendMessage} className="flex items-center gap-2 pt-4">
                    ...
                    />
                    <Button type="submit" size="icon" disabled={isThinking || isCloning || !input.trim()}>
          ...
          <div className="lg:col-span-1 flex flex-col h-[calc(100vh-150px)]">
            <Card>
                ...
                <CardContent className="flex flex-col items-center gap-6">
                    <div className="bg-primary/10 p-6 rounded-full animate-bob">
                    ...
                    <Card className="w-full">
                        <CardHeader>
                            ...
                            <CardDescription>
                                {clonedVoiceId ? "You can upload a new voice anytime." : "Upload a short audio clip of your voice."}
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            ...
                            {isCloning ? 'Cloning Voice...' : (clonedVoiceId ? 'Upload New Voice' : 'Upload Your Voice')}
                            </Button>
                            <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept="audio/*" />
                            {clonedVoiceId && (
                                <div className="mt-4 text-sm text-center p-2 bg-green-100 text-green-800 rounded-md">
                                    <p>Voice successfully cloned!</p>
                                </div>
                            )}
                        </CardContent>
                    </Card>
